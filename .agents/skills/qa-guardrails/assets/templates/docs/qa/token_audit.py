...
DEFAULT_RULES_PATH = Path(__file__).resolve().with_name("token_rules.json")
DEFAULT_VIEWPORTS = [
    (1920, 1080),
    (768, 1024),
    (375, 812),
...
def load_rules(requested_pages: list[str] | None, rules_path: Path) -> dict[str, list[dict[str, str]]]:
    ...
    for entry in report["results"]:
        ...
        if check["status"] == "fail":
            for offender in check["offenders"][:5]:
                lines.append(f"  actual: {offender['actual']} | text=\"{offender['text']}\"")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to token rules JSON file (default: docs/qa/token_rules.json).",
    )
    parser.add_argument(
        "--page",
        action="append",
    ...
    parser.add_argument(
        ...
        type=int,
        default=250,
        help="Post-network-idle delay in milliseconds (default: 250).",
    )
...
def main() -> None:
    ...
    selected_rules = load_rules(args.page, rules_path)
    viewports = build_viewports(args.width)

    server = start_server(args.port)
    try:
        report = run_audit(selected_rules, viewports, args.port, args.settle_ms)
